...
///
/// Yaprflow modification: exposes the existing per-stream C option setter so
/// the bundled multilingual Nemotron model can request automatic language
/// detection. No other upstream wrapper behavior is changed. This file and
/// the modification remain under sherpa-onnx's Apache License 2.0 terms.
...
public class SherpaOnnxRecognizer {
  ...
  /// Set a per-stream runtime option, such as the language prompt used by
  /// multilingual Nemotron models. The option is applied to the recognizer's
  /// currently active stream and should be restored after resetting it.
  public func setOption(key: String, value: String) {
    SherpaOnnxOnlineStreamSetOption(stream, toCPointer(key), toCPointer(value))
  }
