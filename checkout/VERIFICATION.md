...
- Final deployment `dpl_GFHnFQhueD8DKEb9iao8BJ1enJAD` is READY at `https://yaprflow-checkout-7o4gdjjf2-tmoretons-projects.vercel.app`. It is aliased to `yaprflow.com`, publishes the empty Sparkle update feed and update privacy disclosure, and retains the enabled live checkout configuration. Public configuration verifies `enabled: true`, `mode: "live"`, price `{amount: 799, currency: "usd", formatted: "$7.99"}`, `downloadReady: true`, and `voiceDemoAvailable: true`.
