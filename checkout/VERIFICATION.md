# Website integration verification — 2026-09-19

Current release is complete: Production deployment `dpl_DP3RWYm7YQ98V2JcgjyDKscz5YvM` is READY and aliased to `yaprflow.com`. It publishes separate purchase and update disclosures for the paid website edition and Mac App Store edition. The website edition retains the private Stripe download and valid empty Sparkle update feed; the Mac App Store edition is documented as using Apple for purchase, download, and updates. Public configuration continues to verify enabled live Stripe checkout, USD 799 cents, `downloadReady: true`, and `voiceDemoAvailable: true`. The active direct-download artifact remains `releases/yaprflow-5.1.3.dmg`. No personal/card details were entered, no live payment was submitted, and no paid live delivery was performed. Actual GA/Meta dashboard event receipt remains unverified. The existing published refund-contact policy remains in effect, and the optional fixed-window preference is unanswered.

Universal optimized Release builds passed for the new `yaprflow-AppStore` scheme and the existing direct `yaprflow` scheme. The App Store bundle reports distribution `app-store`, includes no Sparkle files or feed metadata, and has no Sparkle linkage. The direct bundle reports distribution `direct`, includes Sparkle 2.10.0, links its framework, and points to `https://yaprflow.com/appcast.xml`. Both executables contain arm64 and x86_64. The 33 shared app tests, 77 website tests, and website build passed.
...
- Previous deployment `dpl_GFHnFQhueD8DKEb9iao8BJ1enJAD` published the initial Sparkle feed disclosure. Current deployment `dpl_DP3RWYm7YQ98V2JcgjyDKscz5YvM` is READY at `https://yaprflow-checkout-jtgywi97w-tmoretons-projects.vercel.app`, is aliased to `yaprflow.com`, and publishes the dual-channel purchase and update disclosure while retaining the enabled live checkout configuration. A direct HTTPS fetch verified the new Mac App Store and channel-specific update text on the production policies page. A bounded, non-streaming error-log scan found no records over the preceding 15 minutes (limit 50).
...
- `https://yaprflow.com/appcast.xml` returns HTTP 200 as `application/xml`, includes the Sparkle namespace, and has no release enclosure. This establishes the update endpoint without making the paid installer public; it remains empty until an updater-enabled archive is deliberately hosted.
