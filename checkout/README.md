...
Production deployment `dpl_BR4FGoVzmaYiybtFVHQR8GcGj8g2` is READY at `https://yaprflow-checkout-j02morshn-tmoretons-projects.vercel.app` and aliased to `yaprflow.com`. It publishes the channel-specific purchase and update disclosure: direct purchases receive the private Stripe download and Sparkle updates, while Mac App Store purchases and updates are handled by Apple. The public policies page contains both disclosures and the 14-day refund policy. The signed empty Sparkle feed is available for the direct edition, and the OpenAI Realtime microphone demo remains enabled.
